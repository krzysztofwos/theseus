...
Update — resolved at the source. The conflation this note diagnoses was removed rather than managed by a projector. The concurrency affordance (`expect_model_hash`) was dropped: it was inert on the agent surface, where the `Session` stamped the hash of the very model it was about to patch, and it guarded a concurrent-writer scenario the single-user CLI does not have. The two remaining CLI affordances were retired with it — `patch`'s single-edit shorthand (the canonical `edit` string is the only form now) and `implement`'s `--body-file` (a multi-line body is passed inline). `PatchRequest` is now `{edit, write}` and `ImplementRequest` is `{method, body}`, identical on every surface, so both project from one clean contract with no per-field reshaping. The generic projector below is no longer needed for today's surfaces. It stays the path to reach for if a concurrent-writer transport — an HTTP or gRPC server — ever reintroduces a genuine per-surface affordance, where the concurrency check belongs to that transport as request metadata. The diagnosis stands as the reason the simplification was the right call.
