...
That proof is deterministic. The initialized foreign-project goal has now also run live and strict green; `init` remains an operator CLI command rather than a tool in the agent catalog, so agent-visible bootstrap is still a separate claim.
