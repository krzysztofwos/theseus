...
- `theseus-agent` (L4) — `rust/agent/`. The `Agent` inbound, the binary `agent`. An LLM drives the service's operations as tools over a `Session` in a loop, behind the generated `Llm` port — an Anthropic adapter, or an offline stub — for at most the generated `TURN_BUDGET` turns; the port, its `Turn` request, and the budget render from the inbound's modeled interior. A loop-level `restart` tool rebuilds the workspace and re-enters the session in the new binary, over a transcript persisted to `.theseus/session.json` (resumed with `agent --resume`).
...
9. Flow conformance — every authored handler reaches exactly the ports its operation's `uses` edges declare (a functor from the declared flow graph into the one extracted from the handlers). A declared port the handler never touches and a touched port the operation never declares both fail.
10. Interior coverage — every adapter of an inbound's interior port authors every port method, so a loop adapter (the offline stub beside the real model adapter) never silently falls to the trait default when its port grows.
