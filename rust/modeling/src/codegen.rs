...
fn render_http_module(inbound: &Inbound, service: &Service, model: &Model) -> TokenStream {
    ...
    let parsers: Vec<TokenStream> = service
        ...
        .map(|def| {
            ...
            let fn_name = format_ident!("parse_{}_http", def.name.to_lowercase());
...
/// struct the service's operations take, a wrapper message per scalar response
/// label, and the service with one rpc per operation. The build compiles it, so
/// the wire schema is a projection of the model like every other surface.
...
/// `value` for a label that resolves to a string, `Empty` for none, or the
/// struct rendered field by field.
fn proto_response_message(op: &Operation, model: &Model) -> (String, String) {
    ...
    match rust_type(&op.response, model).as_str() {
        ...
        _ => match model.type_def(&op.response).map(|def| &def.shape) {
            ...
            _ => panic!(
                "the gRPC renderer does not yet cover the response type `{}`",
                op.response
            ),
        },
    }
...
/// The snake-case form the proto build gives a service's server module: a word
/// boundary at each uppercase letter, lowercased.
...
/// The proto type a contract type label maps to.
fn proto_type(label: &str, model: &Model) -> String {
    if let Some(inner) = optional_inner(label) {
        return format!("optional {}", proto_type(inner, model));
    }
    ...
    match label {
        ...
        other => match model.type_def(other).map(|def| &def.shape) {
            Some(TypeShape::Struct(_)) => other.to_string(),
    ...
    let methods: Vec<TokenStream> = service
        ...
        .map(|op| {
            ...
            quote! {
                async fn #method(
                    &self,
                    request: tonic::Request<proto::#request_msg>,
