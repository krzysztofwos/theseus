...
Every session also carries an immutable `ProjectContext`: a canonical operator-selected root, initial model, stable project ID, and versioned Rust workspace layout. Filesystem, Cargo, and checkpoint ports report their binding, which is compared with the session before use; resumable state carries the same binding. Built-in HTTP, gRPC, MCP, CLI, and agent compositions derive every adapter from one context. The selected pathname is revalidated before adapter access, the root is not a tool input, and neither the root nor layout can change during a session. A foreign project therefore gets the same `query` → `patch` → `implement` → `check` → `test` → `verify` → `rollback` loop without inheriting Theseus's compile-time root or path conventions.
