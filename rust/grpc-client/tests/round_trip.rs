...
impl Checkpoint for ModelCheckpoint {
    ...
    async fn snapshot(
        ...
        *self.0.lock().expect("the checkpoint mutex is available") = Some(request.model.clone());
        Ok(CheckpointSnapshot {
            reference: "grpc-stateful-snapshot".to_string(),
        })
    }
    ...
    async fn restore(
        ...
        Ok(CheckpointRestore {
            detail: "restored grpc-stateful-snapshot".to_string(),
            model,
        })
    }
}
...
#[tokio::test]
async fn checkpoint_lifecycle_requests_cross_grpc() {
    ...
    let diff = client
        ...
        .await
        .expect("the diff reference crosses gRPC");
    assert_eq!(diff, "diffed snapshot-a");

    let pruned = client
        .prune(SnapshotRetention { keep: 7 })
        .await
        .expect("the retention limit crosses gRPC");
    ...
    let released = client
        ...
        .await
        .expect("the release reference crosses gRPC");
    assert_eq!(released, "released snapshot-b");
}

#[tokio::test]
async fn rollback_adopts_the_snapshot_model_across_grpc() {
    ...
    let reference = client
        .snapshot(SnapshotRequest {
            label: "before gRPC speculation".to_string(),
        })
        .await
        .expect("the snapshot crosses gRPC");
    client
        .patch(PatchRequest {
            edit: vec![Edit::Add {
                parent: "model:theseus".to_string(),
                kind: "type".to_string(),
                name: "GrpcAfterSnapshot".to_string(),
                attrs: [("shape".to_string(), "foreign:String".to_string())].into(),
            }],
            write: false,
        })
        .await
        .expect("the speculative patch crosses gRPC");
    let speculative = client
        .query(QueryRequest {
            find: Some("GrpcAfterSnapshot".to_string()),
            node: None,
            kind: Some("type".to_string()),
        })
        .await
        .expect("the speculative model is queried over gRPC");
    assert_eq!(speculative.handles.len(), 1);

    client
        .rollback(SnapshotRef { reference })
        .await
        .expect("the rollback crosses gRPC");
    let restored = client
        .query(QueryRequest {
            find: Some("GrpcAfterSnapshot".to_string()),
            node: None,
        ...
        .await
        .expect("the restored model is queried over gRPC");
